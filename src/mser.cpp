...
variation_(numeric_limits<double>::infinity()), stable_(false), parent_(0), child_(0), next_(0), start_x_(-1), end_x_(-1), start_y_(-1), end_y_(-1), uid_(-1), parent_uid_(-1)
{
...
inline void MSER::Region::accumulate(int x, int y)
{
    ...
    if (start_x_ == -1) {
        ...
        start_x_ = MIN(start_x_, x);
        end_x_   = MAX(end_x_, x);
        start_y_ = MIN(start_y_, y);
        end_y_   = MAX(end_y_, y);
    }
}
...
void MSER::Region::merge(Region * child)
{
    ...
    if (start_x_ != -1 && child->start_x_ != -1) {
        start_x_ = MIN(start_x_, child->start_x_);
        start_y_ = MIN(start_y_, child->start_y_);
        end_x_   = MAX(end_x_,   child->end_x_);
        end_y_   = MAX(end_y_,   child->end_y_);
    } else if (start_x_ == -1 && child->start_x_ != -1) {
        start_x_ = child->start_x_;
        start_y_ = child->start_y_;
        end_x_   = child->end_x_;
        end_y_   = child->end_y_;
    }
...
void MSER::Region::save(double minDiversity, vector<Region> & regions, int parent_uid)
{
	if (stable_) {
		...
		if (stable_) {
			...
			regions.back().parent_uid_ = parent_uid;
	...
	for (Region * child = child_; child; child = child->next_)
		child->save(minDiversity, regions, parent_uid);
}
...
void MSER::operator()(const uint8_t * bits, int width, int height, vector<Region> & regions)
{
    ...
    regionStack.back()->uid_ = poolIndex_ - 1;
    assert(regionStack.back()->uid_ >= 0);
