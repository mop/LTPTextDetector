...
static bool MSERStableCheck( MSERConnectedComp* comp, MSERParams params )
{
    ...
    int stable = ( (params.maxEvolution == 0.0 || (dvar && !comp->dvar)) && comp->var < params.maxVariation && div > params.minDiversity );
    comp->var = var;
    comp->dvar = dvar;
    if ( stable )
...
#endif
            {
                ...
                if ( pixel_val < comptr[-1].grey_level )
                {
                    ...
                    if ( MSERStableCheck( comptr, params ) )
                    {
                        ...
                        //double var = MSERVariationCalc(comptr, params.delta);
                        double tmp = comptr->var;
                        cvSeqPush( vars, &tmp);
                    }
             ...
             MSERParams params )
{
    ...
    extractMSER_8UC1_Pass( ioptr, imgptr, heap_start, pts, history, comp, step, stepmask, stepgap, params, -1, contours, vars, storage );
           ...
           MSERParams params )
{
    ...
    vars = *_vars = cvCreateSeq( 0, sizeof(CvSeq), sizeof(CvSeq*), storage );
    ...
    {
        case CV_8UC1:
            extractMSER_8UC1( src, mask, contours, vars, storage, params );
            break;
        case CV_8UC3:
            extractMSER_8UC3( src, mask, contours, vars, storage, params );
            break;
    }
}


HierarchicalMSER::HierarchicalMSER( int _delta, int _min_area, int _max_area,
      double _max_variation, double _min_diversity, bool _stable)
...
void HierarchicalMSER::operator()( const Mat& image, vector<vector<Point> >& dstcontours, vector<double> &dstvars, vector<Vec4i>& hierarchy, const Mat& mask ) const
{
    ...
    Seq<CvSeq*> contours;
    Seq<double> vars;
    extractMSER( &_image, pmask, &contours.seq, &vars.seq, storage,
                 MSERParams(delta, minArea, maxArea, maxVariation, minDiversity,
                            stable, areaThreshold, minMargin, edgeBlurSize));
    SeqIterator<CvSeq*> it = contours.begin();
    SeqIterator<double> it2 = vars.begin();
    size_t i, ncontours = contours.size();
    dstcontours.resize(ncontours);
    dstvars.resize(ncontours);
    for( i = 0; i < ncontours; i++, ++it, ++it2 ) {
...
void HierarchicalMSER::detectImpl( const Mat& image, vector<KeyPoint>& keypoints, const Mat& mask ) const
{
    vector<vector<Point> > msers;
    vector<Vec4i> hierarchy;
    vector<double> vars;

    (*this)(image, msers, vars, hierarchy, mask);
